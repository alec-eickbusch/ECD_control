...
class OptimalCorrections:
    ...
    def grid_search(self, pts=11):
        alphas_real = np.linspace(
            self.alpha - self.d_alpha, self.alpha + self.d_alpha, pts
        )
        alphas_imag = np.linspace(
            self.alpha - self.d_alpha, self.alpha + self.d_alpha, pts
        )
        ...
        print(
            "d_alpha: %.3f d_theta: %.3f grid pts: %dx%dx%d (%d total)"
    ...
    def opt_correction2(self):
        init_params = np.array(
            [
                np.real(self.initial_alpha),
                np.imag(self.initial_alpha),
                self.initial_theta,
            ]
        )
        ...
        def callback_fun(x, f, accepted):
            ...
            print(
                "alpha: %.3f + %.3fj, thata deg: %.3f"
                % (x[0], x[1], x[2] * 180.0 / np.pi)
            )
