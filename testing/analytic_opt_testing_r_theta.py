...
real_der_num = ((cd_grape_obj.D(alpha_new) - cd_grape_obj.D(alpha))/dalpha_r)
real_der_ana = cd_grape_obj.dalpha_r_dD(alpha)
...
real_der_num = ((cd_grape_obj.D(alpha_new) - cd_grape_obj.D(alpha))/dalpha_theta)
real_der_ana = cd_grape_obj.dalpha_theta_dD(alpha)
diff = real_der_num - real_der_ana
...
real_der_num = ((cd_grape_obj.CD(beta_new) - cd_grape_obj.CD(beta))/np.abs(dbeta_r))
real_der_ana = cd_grape_obj.dbeta_r_dCD(beta)
diff = real_der_num - real_der_ana
...
real_der_num = ((cd_grape_obj.CD(beta_new) - cd_grape_obj.CD(beta))/np.abs(dbeta_theta))
real_der_ana = cd_grape_obj.dbeta_theta_dCD(beta)
diff = real_der_num - real_der_ana
diff = diff[:10, :10]  # they differ near the edges of the hilbert space
print(repr(diff))
print(np.max(np.abs(diff)))
