...
initial_state = qt.tensor(
    qt.basis(N, 0), (qt.basis(N2, 0) + qt.basis(N2, 1)) / np.sqrt(2)
)
CD_control_obj = CD_control(initial_state, N_blocks=N_blocks)
sys = System(
    chi=chi,
    Ec=Ec,
    alpha0=alpha0,
...
num_test = 4
betas = [
    np.random.uniform(low=-3, high=3) + 1j * np.random.uniform(low=-2, high=2)
...
for beta, phi, theta in zip(betas, phis, thetas):
    desired_state = (
        CD_control_obj.CD(beta) * CD_control_obj.R(phi, theta) * initial_state
    )
    ...
    CD_control_obj.phis = [phi, 0]
    CD_control_obj.thetas = [theta, 0]
