...
beta_new[k] = (beta_r_k)*np.exp(1j*(beta_theta_k + dx))
...
print(dbeta_theta[k])
print("Precentage Diff: " + str((dbeta_theta[k] - num_gradient)/num_gradient*100) + "%")
...
print(fid)
print(afid)
print(adbeta_theta[k])
print("Precentage Diff: " + str((adbeta_theta[k] - num_gradient)/num_gradient*100) + "%")

# %% Approx Unitary Fidelity (with eigenstates)
unitary_initial_states = []
unitary_initial_state_weights = np.array(CD_control_obj.target_unitary.eigenstates()[0])
n = N*N2
# n = N*N2*5//8
...
estates = CD_control_obj.target_unitary.eigenstates()[1]
for i in sorted_ind:
